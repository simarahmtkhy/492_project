...
class InstructGenerator:
    ...
    async def process_batch(self, batch, switch_count=0):
        ...
        try:
            os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = api_keys[switch_count]
            model = VertexAI(**self.model_args)
            predictions = await model.abatch(inputs)
...
async def main():
    ...
    parser.add_argument('--max_size', type=int, default=90000)
